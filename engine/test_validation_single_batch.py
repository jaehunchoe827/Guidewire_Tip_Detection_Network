...
def validation(config, weights_epoch):
    ...
    with torch.no_grad():
        ...
        # With x_val, y_val already moved to CUDA
        def report(name, t):
            fin = t.isfinite()
            print(f"{name}: shape={tuple(t.shape)}, finite={fin.all().item()}, "
                f"nan={torch.isnan(t).any().item()}, inf={torch.isinf(t).any().item()}")

        # Inputs
        report("x_val", x_val)

        # Stage 0: grayscale->RGB expansion the model does internally
        x_rgb = x_val.repeat(1, 3, 1, 1)
        report("x_rgb", x_rgb)

        # Stage 1: backbone
        p3, p4, p5 = model.net(x_rgb)
        report("net.p3", p3); report("net.p4", p4); report("net.p5", p5)

        # Stage 2: FPN
        fp3, fp4, fp5 = model.fpn((p3, p4, p5))
        report("fpn.p3", fp3); report("fpn.p4", fp4); report("fpn.p5", fp5)

        # Stage 3: head (prepare input as model.forward does)
        z = [x_rgb, fp3, fp4, fp5]
        pred_val = model.head(z)  # (B, H, W)
        report("head.out", pred_val)

        bad = []
        for n, p in model.head.named_parameters():
            if torch.isnan(p).any() or torch.isinf(p).any():
                bad.append(("param", n))
        for n, b in model.head.named_buffers():
            if torch.isnan(b).any() or torch.isinf(b).any():
                bad.append(("buffer", n))
        print("head bad tensors:", bad)
    ...
    val_losses_avg = {loss_name: loss_sum
